...
def test_missing_goplus():
    ...
    try:
        ...
        assert card['data']['goplus']['risk_source'] == 'unavailable', "GoPlus should be unavailable"
        assert card['meta'].get('degrade') == True, "Should be degraded"
        assert 'missing goplus' in card['data']['rules'].get('reasons', []), "Should have degrade reason"
