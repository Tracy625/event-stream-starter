...
def verify_card_preview(event_key: str, base_url: str = "http://localhost:8000") -> dict:
    """
    ...
    params = {
        "event_key": event_key,
        "render": 1  # Always test with render enabled
    }
    ...
    try:
        ...
        schema, resolver = schema_and_resolver if isinstance(schema_and_resolver, tuple) else (schema_and_resolver, None)
        try:
            if resolver:
        ...
        result = {
            "pass": True,
            "summary_backend": card_data.get("meta", {}).get("summary_backend", "unknown"),
        ...
        if result["pass"]:
            ...
            if summary_len > max_summary:
                ...
                result["pass"] = False
                result["reason"] = f"Risk note too long: {risk_note_len} > {max_risk_note}"
        ...
        if timeout_ms <= 1:
            ...
            if result.get("summary_backend") != "template":
                result["timeout_degrade_check"] = "Expected template backend with low timeout"
