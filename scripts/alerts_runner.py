...
class MetricsParser:
    ...
    def parse_metrics(text: str) -> Dict[str, Dict[str, float]]:
        ...
        for line in text.split('\n'):
            ...
            match = re.match(r'^([a-zA-Z_:][a-zA-Z0-9_:]*(?:_bucket|_count|_sum|_total)?)\{?(.*?)\}?\s+([0-9.\-+eE]+)$', line)
            if match:
                try:
...
class AlertState:
    ...
    def update_breach(self, rule_name: str, breached: bool, window_seconds: int) -> bool:
        """
...
class AlertEvaluator:
    ...
    def evaluate_rule(self, rule: Dict[str, Any], metrics: Dict[str, Dict[str, float]]) -> tuple[bool, str]:
        """
        ...
        if expr == 'error_rate':
            ...
            err_delta = self.state.get_delta(f'telegram_send_total_{err_key}', err_value)
...
class AlertNotifier:
    ...
    def __init__(self, webhook_url: Optional[str] = None, notify_script: Optional[str] = None):
        self.webhook_url = webhook_url
        self.notify_script = notify_script

    def notify(self, rule_name: str, severity: str, reason: str, dry_run: bool = False) -> bool:
        """
    ...
    def evaluate_once(self):
        ...
        for rule in self.rules:
            ...
            if should_fire:
                ...
                print(f"alert.fired: name={rule_name} severity={severity} "
...
def main():
    ...
    parser.add_argument('--min-breach-seconds', type=int, default=60,
                        help='Global minimum debounce window')
    parser.add_argument('--silence-seconds', type=int, default=300,
                        help='Global default silence window')
    ...
    # State and options
    parser.add_argument('--state-file', default='.alerts_state.json',
                        help='State file path')
    parser.add_argument('--dry-run', action='store_true',
                        help='Print only, no notifications')
