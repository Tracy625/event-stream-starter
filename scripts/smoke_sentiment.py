...
TEST_INPUTS = ["I love this", "this is terrible", "great job", "awful mess"]
...
def _assert_degrade(payload: Dict[str, Any], expected_reason: str | tuple[str, ...]):
    ...
    if payload.get("reason") not in reasons:
        print(
            f"Unexpected degrade reason: {payload.get('reason')} (want {reasons})",
            file=sys.stderr,
        )
        sys.exit(1)
...
def process_batch_rules(texts: List[str]) -> List[Dict[str, Any]]:
    ...
    for text in texts:
        try:
            ...
            results.append({"label": "neu", "score": 0.0, "error": str(e)})
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["hf", "rules"],
        default="hf",
        help="Backend to use (hf or rules)",
    )
    parser.add_argument(
        "--summary-json",
        action="store_true",
