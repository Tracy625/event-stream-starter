...
from api.events import make_event_key, merge_event_evidence, _build_evidence_item
...
def verify_event_key_consistency(events: List[Dict[str, Any]]) -> bool:
    ...
    for event in events:
        ...
        if key1 != key2:
            errors.append(f"Non-deterministic key for event: {event.get('id', 'unknown')}")
            continue
    ...
    for event in events:
        ...
        if event.get("source") == "x" or event.get("has_x_data"):
            x_evidence = _build_evidence_item(
                ...
                ref={"tweet_id": event.get("tweet_id", "123"), "author": event.get("author", "user")},
        ...
        if event.get("token_ca"):
            ...
            goplus_evidence = _build_evidence_item(
                source="goplus",
                ts=event.get("created_ts", datetime.now(timezone.utc)),
                ref={"goplus_endpoint": "/api/v1/token_security", "chain_id": "1", "address": event.get("token_ca")},
                summary="Risk: low",
                weight=0.9
            )
        ...
        if result["deduped"] != len(evidence_items):
            errors.append(f"Event {event_key[:8]}: Expected {len(evidence_items)} deduped, got {result['deduped']}")
        
        # Verify count consistency
        if result["after_count"] != len(evidence_items):
            errors.append(f"Event {event_key[:8]}: Expected {len(evidence_items)} after merge, got {result['after_count']}")
    
    # Print statistics
    if stats["total_events"] > 0:
        print(f"  Total events: {stats['total_events']}")
        print(f"  Multi-source events: {stats['multi_source_events']}")
        print(f"  Cross-source co-occurrence (x+dex/goplus): {stats['cross_source_cooccurrence']}")
        print(f"  Average evidence per event: {stats['total_evidence'] / stats['total_events']:.2f}")
        if stats['total_evidence'] + stats['deduped_evidence'] > 0:
            dedup_rate = stats['deduped_evidence'] / (stats['total_evidence'] + stats['deduped_evidence']) * 100
...
def verify_minimum_refs(events: List[Dict[str, Any]], min_refs: int = 2) -> bool:
    ...
    for event in events:
        ...
        if ref_count < min_refs:
            ...
            events_with_insufficient_refs.append({
                "event": event_id,
                "refs": ref_count
            })
    
    if events_with_insufficient_refs:
        print(f"  ❌ {len(events_with_insufficient_refs)} events have < {min_refs} refs:")
...
def main():
    ...
    if all(checks):
        ...
        stats = {
            ...
            "multi_source_events": merge_stats.get("multi_source_events", 0) if merge_stats else 0,
            "cross_source_cooccurrence": merge_stats.get("cross_source_cooccurrence", 0) if merge_stats else 0,
