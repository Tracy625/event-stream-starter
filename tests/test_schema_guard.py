...
def test_no_duplicate_route_prefix():
    ...
    assert len(duplicate_routes) == 0, f"Found duplicate route prefixes: {duplicate_routes}"
...
def test_signals_table_required_columns():
    ...
    existing_columns = [line.strip() for line in result.stdout.split('\n') if line.strip()]
    ...
    assert len(missing_columns) == 0, f"Missing required columns in signals table: {missing_columns}"
...
def test_decimal_precision():
    ...
    for input_val, expected in test_cases:
        ...
        assert result == expected, f"serialize_decimal({input_val}) = {result}, expected {expected}"
