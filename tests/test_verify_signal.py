...
class TestVerifySignal:
    ...
    def test_update_upgrade(self, mock_db, mock_redis, mock_rules, mock_signal):
        ...
        with patch(
            "worker.jobs.onchain.verify_signal.fetch_onchain_features"
        ) as mock_fetch:
            with patch("worker.jobs.onchain.verify_signal.evaluate") as mock_eval:
                ...
                mock_eval.return_value = Verdict(
                    decision="upgrade", confidence=1.0, note=None
                )
                ...
                )  # UPDATE signals + INSERT signal_events
    ...
    def test_downgrade_priority(self, mock_db, mock_redis, mock_rules, mock_signal):
        ...
        with patch(
            ...
            with patch("worker.jobs.onchain.verify_signal.evaluate") as mock_eval:
                ...
                mock_eval.return_value = Verdict(
                    decision="downgrade", confidence=1.0, note=None
                )
    ...
    def test_insufficient_no_state_change(
        self, mock_db, mock_redis, mock_rules, mock_signal
    ):
    ...
    def test_evidence_delayed_timeout(
        self, mock_db, mock_redis, mock_rules, mock_signal
    ):
            ...
            with patch(
                ...
                with patch("worker.jobs.onchain.verify_signal.evaluate") as mock_eval:
                    ...
                    result = module.process_candidate(
                        mock_db, mock_signal, mock_rules, mock_redis
                    )
                    ...
                    update_call = mock_db.execute.call_args_list[0]
                    assert (
                        update_call[0][1]["state"] == "candidate"
                    )  # State not changed
    ...
    def test_bq_cost_metrics(self):
        ...
        with patch("worker.jobs.onchain.verify_signal.BQProvider") as mock_provider:
            ...
            mock_instance.run_template.return_value = {
                ...
                "metadata": {"total_bytes_processed": 1048576},  # 1MB
            }
    ...
    def test_skip_recent(self, mock_db, mock_redis, mock_rules):
        ...
        signal.event_key = "eth:0x1234:1234567890"
        signal.state = "candidate"
        signal.created_at = datetime.now(timezone.utc) - timedelta(
            seconds=30
        )  # Only 30 seconds old
    ...
    def test_run_once_stats(self, mock_redis_fn, mock_load_rules, mock_get_db):
        ...
        with patch(
            "worker.jobs.onchain.verify_signal.process_candidate"
        ) as mock_process:
