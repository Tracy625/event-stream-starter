...
class TestTopicPipelineIntegration:
    ...
    def test_end_to_end_topic_flow(self, session, monkeypatch):
        ...
        """
            ),
            {
                ...
                "type": "market-update",
                "hash1": "t.topic1",
                "entities1": json.dumps(["pepe", "gem"]),
                "score1": 0.8,
                "ts1": now,
                "key2": "event_002",
                "type": "market-update",
                "hash1": "t.topic1",
                "entities2": json.dumps(["pepe"]),
                "score2": 0.7,
                "ts2": now - timedelta(minutes=5),
                "key3": "event_003",
                "type": "market-update",
                "hash2": "t.topic2",
                "entities3": json.dumps(["arb"]),
                "score3": 0.6,
        ...
        monkeypatch.setattr(
            "worker.jobs.topic_signal_scan.get_db_session", lambda: session
        )
        ...
        monkeypatch.setattr(
            "worker.jobs.topic_aggregate.get_db_session", lambda: session
        )
            ...
            candidates.append(
                {
                    ...
                    "entities": (
                        json.loads(row["all_entities"]) if row["all_entities"] else []
                    ),
    ...
    def test_rollback_on_error(self, session, monkeypatch):
        ...
        monkeypatch.setattr(
            "worker.jobs.topic_signal_scan.get_db_session", lambda: failing_session
        )
