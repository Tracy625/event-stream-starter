...
def test_rate_limit_429():
    ...
    from api.routes_expert_onchain import get_expert_onchain, check_rate_limit
    from fastapi import HTTPException
...
def test_cache_hit_and_ttl_decrease():
    ...
    with patch.dict(os.environ, {
        ...
        with patch('api.routes_expert_onchain.get_redis', return_value=mock_redis):
            with patch('api.routes_expert_onchain.check_rate_limit', return_value=False):
...
def test_pg_series_ok():
    ...
    result = fetch_series_pg("eth", "0x1111111111111111111111111111111111111111", mock_db)
    ...
    assert result["overview"]["top10_share"] == 0.360  # Latest value, clamped and rounded
        ...
        with patch('api.routes_expert_onchain.get_redis', return_value=mock_redis):
            with patch('api.routes_expert_onchain.check_rate_limit', return_value=False):
                ...
                with patch('api.providers.onchain.bq_provider.BQProvider') as mock_bq:
                    mock_bq.return_value.query_light_features.side_effect = Exception("BQ Error")
                    ...
                    cached_data = {
                        ...
                        "address": "0x1111111111111111111111111111111111111111",
                        "series": {
                            "h24": {"w30": [{"ts": "2025-01-01T00:00:00Z", "addr_active": 100}], "w60": []},
                            "d7": {"w30": [], "w60": []}
                        },
...
def test_no_event_key_usage():
    ...
    assert "event_key" not in source_code.lower(), "Implementation should not reference event_key"
