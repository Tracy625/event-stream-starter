...
def seed_demo_data(db_sessionmaker, demo_keys):
    with with_session(db_sessionmaker) as s:
        ...
        s.execute(sa_text("DELETE FROM signals WHERE event_key IN (:k1,:k2,:k3);"),
                  dict(k1=demo_keys["DEMO1"], k2=demo_keys["DEMO2"], k3=demo_keys["DEMO3"]))
        s.execute(sa_text("""
    ...
    with with_session(db_sessionmaker) as s:
        s.execute(sa_text("DELETE FROM signals WHERE event_key IN (:k1,:k2,:k3);"),
                  dict(k1=demo_keys["DEMO1"], k2=demo_keys["DEMO2"], k3=demo_keys["DEMO3"]))
        s.execute(sa_text("DELETE FROM events  WHERE event_key IN (:k1,:k2,:k3);"),
                  dict(k1=demo_keys["DEMO1"], k2=demo_keys["DEMO2"], k3=demo_keys["DEMO3"]))
        s.commit()
...
def _assert_basic_shape(j):
    ...
    assert isinstance(j["all_reasons"], list) and len(j["all_reasons"]) >= len(j["reasons"])
