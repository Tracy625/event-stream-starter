...
def test_confidence_precision():
    ...
    if result["verdict"]["confidence"] != 0:
        ...
        if '.' in confidence_str:
            ...
            assert len(decimal_part) <= 3, f"Confidence {result['verdict']['confidence']} has more than 3 decimal places"
    
    # Check onchain numeric fields precision
    if result["onchain"]:
        for key in ["active_addr_pctl", "growth_ratio", "top10_share", "self_loop_ratio"]:
