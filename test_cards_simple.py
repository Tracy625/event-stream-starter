...
def show_curl_commands():
    ...
    print('curl -s "http://localhost:8000/cards/preview?event_key=ETH:TOKEN:0X123456" | jq')
    
    print("\n# 2. Preview with render:")
    print('curl -s "http://localhost:8000/cards/preview?event_key=ETH:TOKEN:0X123456&render=1" | jq')
    
    print("\n# 3. Test degraded case (as per acceptance criteria):")
    print('curl -s "http://localhost:8000/cards/preview?event_key=TEST_BAD&render=1" | jq')
