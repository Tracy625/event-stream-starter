...
class RulesRefiner(RefinerBackend):
    def refine(self, evidence_texts: List[str], *, hint: Optional[str] = None) -> dict:
        ...
        low = text.lower()
        if "launch" in low:
            reasons.append("New token/project launch")
        if "listing" in low or "list" in low or "pair" in low:
            reasons.append("Exchange listing or pair opened")
        if not reasons:
            reasons = ["Heuristic summary"]
...
class LLMRefiner(RefinerBackend):
    ...
    def refine(self, evidence_texts: List[str], *, hint: Optional[str] = None) -> dict:
        ...
        fallbacks: Sequence[str] = os.getenv(
            ...
            for attempt in range(max_retries + 1):
                try:
                    messages = [
                        ...
                        {
                            "role": "user",
                            "content": f"hint={hint or ''}\nEVIDENCE:\n{joined}",
                        },
                    ]
                    resp = client.chat.completions.create(
                        **_build_kwargs(model, messages)
                    )
                    raw = resp.choices[0].message.content
                    data = json.loads(raw)
                    if isinstance(data.get("summary"), str):
                        data["summary"] = data["summary"][
                            :REFINE_SUMMARY_MAXLEN
                        ].rstrip()
                    out = RefineModel(**data).dict()
                    log_json(
                        stage="refine.success",
                        backend="llm",
                        model=model,
                    ...
                    msg = str(e)
                    if "Unsupported value: 'temperature'" in msg and model.startswith(
                        "gpt-5"
                    ):
                        try:
                            resp = client.chat.completions.create(
        ...
        log_json(
            stage="refine.degrade",
            reason=str(last_exc or "unknown"),
            latency_ms=int((time.time() - t0) * 1000),
        )
        return RulesRefiner().refine(evidence_texts, hint=hint)
...
def refine_evidence(evidence_texts: List[str], *, hint: Optional[str] = None) -> dict:
    # 打印一次模块路径，防止再跑错文件
    log_json(
        stage="refine.request",
        n_inputs=len(evidence_texts),
