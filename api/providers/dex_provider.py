...
class DexProvider:
    ...
    def __init__(
        ...
        self.timeout_s = float(
            timeout_s if timeout_s is not None else (env_timeout or 1.5)
        )
    ...
    def _cache_key(
        self, chain: str, contract: str, bucket: Optional[int] = None
    ) -> str:
    ...
    def _set_cache(self, key: str, data: Dict[str, Any], ttl: Optional[int] = None):
        ...
        self._memory_cache[key] = {"ts": time.time(), "data": data}
    ...
    def _fetch_dexscreener(self, chain: str, contract: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log_json(
                stage="dex.request", source="dexscreener", chain=chain, contract=ca_norm
            )
            ...
            if data and "pairs" in data and len(data["pairs"]) > 0:
                ...
                pairs = [
                    p for p in data["pairs"] if p.get("chainId", "").lower() == chain_id
                ]
                if not pairs:
                    pairs = data["pairs"]
    ...
    def get_snapshot(self, chain: str, contract: str) -> Dict[str, Any]:
        ...
        try:
            ...
            primary_reason = self._map_err_reason(
                e, getattr(e.response, "status_code", None)
            )
            log_json(
                stage="dex.fallback",
        ...
        if not result:
            try:
                ...
                secondary_reason = self._map_err_reason(
                    e, getattr(e.response, "status_code", None)
                )
                log_json(
            ...
            last_ok.update(
                {
                    ...
                    "notes": (last_ok.get("notes") or [])
                    + [f"last_ok_from:{src_prev}"],
                }
            )
