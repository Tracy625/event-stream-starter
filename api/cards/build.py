    ...
    for source in sources:
        if source:
            ts = _extract_timestamp(
                source, ["as_of", "ts", "updated_at", "created_at", "timestamp"]
            )
            if ts:
...
def _build_dex_section(dex_data: Optional[Dict]) -> Optional[Dict]:
    ...
    for timeframe in ["m5", "m15", "h1"]:
        if timeframe in dex_data:
            ...
            if isinstance(frame_data, dict) and all(
                k in frame_data for k in ["open", "high", "low", "close"]
            ):
                ohlc[timeframe] = {
...
def _build_rules_section(
    rules_data: Optional[Dict], degrade_reasons: List[str]
) -> Dict:
...
def build_card(event_key: str, render: bool = False) -> Dict:
    ...
    if goplus_section:
        ...
        data["goplus"] = {"risk": "gray", "risk_source": "unavailable"}
        degrade = True
