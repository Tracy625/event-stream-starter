...
def _render_template(symbol: str, price: Optional[float], liq: Optional[float], 
                    level: str, risk: str) -> Tuple[str, str]:
...
def _resolve_refiner() -> tuple[Optional[Any], str]:
    ...
    if not class_path:
        ...
        candidates = ["api.refine:MiniRefiner", "api.hf_adapter:HFRefiner", "api.refiner:RefinerClient"]
    else:
...
def _call_refiner(payload: dict, timeout_ms: int, max_sum: int, max_note: int) -> Optional[Tuple[str, str, str]]:
    """
...
def summarize_card(input_payload: dict, timeout_ms: Optional[int] = None) -> Tuple[str, str, dict]:
    """
    ...
    if backend == "llm" and timeout_ms > 1:
        llm_result = _call_refiner(input_payload, timeout_ms, max_summary_chars, max_risknote_chars)
        if llm_result:
    ...
    try:
        ...
        logger.info(json.dumps({"evt": "cards.summarize", **log_data}, ensure_ascii=False))
    except Exception:
        pass
    
    meta_extra = {
        "summary_backend": actual_backend,
