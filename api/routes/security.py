...
async def check_token_security(
    ...
    log_json(
        stage="security.api.request",
        endpoint="token",
        params={"chain_id": chain_id, "address": address, "raw": raw},
    )
    try:
...
async def check_address_security(
    ...
    log_json(
        stage="security.api.request",
        endpoint="address",
        params={"address": address, "raw": raw},
    )
    try:
        provider = GoPlusProvider()
...
async def check_approval_security(
    ...
    type: str = Query(
        "erc20", description="Token type", pattern=r"^(erc20|erc721|erc1155)$"
    ),
    raw: bool = Query(False, description="Include raw response data"),
):
    ...
    log_json(
        stage="security.api.request",
        endpoint="approval",
        params={"chain_id": chain_id, "address": address, "type": type, "raw": raw},
    )
    try:
        provider = GoPlusProvider()
