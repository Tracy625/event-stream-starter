...
def _record_outbox_backlog_metric() -> None:
    ...
    try:
        ...
        Exception
    ) as exc:  # pragma: no cover - metrics endpoint should degrade quietly
...
def _record_beat_metrics() -> None:
    ...
    if last is None:
        log_json(
            stage="metrics.beat_heartbeat_missing", message="No heartbeat recorded"
        )
        register_gauge(
...
def _record_total_apis_gauge() -> None:
    ...
    if redis is None:
        register_gauge(
            "total_apis", "Total successful outbound API calls in the last 24h", value=0
        )
        return
    ...
    try:
        ...
        if build_version or build_commit:
            register_gauge(
                "build_info",
                "Build information",
                labels={"version": build_version or "", "commit": build_commit or ""},
                value=1,
            )
        ...
        register_counter(
            "telegram_retry_total", "Total number of Telegram send retries", value=0
        )
        register_counter(
            "cards_degrade_count", "Total number of degraded events", value=0
        )
        ...
        try:
            ...
            metrics_text = (
                "# HELP up 1 if metrics handler is healthy\n# TYPE up gauge\nup 0\n"
            )
