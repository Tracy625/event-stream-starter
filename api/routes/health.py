...
def readyz():
    ...
    try:
        with with_db() as db:
            ...
            try:
                enqueue(db, channel_id=0, thread_id=None, event_key="readyz_probe", payload_json={"ok": True})
    ...
    log_json(stage="readyz.ok", level="info", operation="readyz", status="ready", latency=latency_ms)
    # Observe latency in histogram
    try:
        readyz_latency_ms.observe(latency_ms)
    except Exception:
        pass
    return JSONResponse({"status": "ready", "latency_ms": latency_ms}, headers={"Cache-Control": "no-store"})
