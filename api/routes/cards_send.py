...
from api.core.cache_keys import DEDUP_TTL, dedup_key, idemp_key
...
DEGRADE_COUNTER = metrics.counter("cards_degrade_count", "cards degrade batches")
...
EXTERNAL_ERR_NET = metrics.counter(
    "external_error_total_net", "external errors: network/timeouts"
)
...
async def send_card(
    ...
    if dry_run == 1:
        ...
        for i in range(count):
            items.append(
                {"ok": False, "dry_run": True, "event_key": event_key, "attempt": 0}
            )
            failed += 1
    ...
    for i in range(count):
        ...
        if success:
            ...
            try:
                ...
                logger.info(
                    f"telegram send failed trace_id={tid} event_key={event_key} error_code={error_code}"
                )
