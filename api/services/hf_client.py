...
class HfClient:
    ...
    def __init__(self, *, backend: Optional[str] = None, model_id: Optional[str] = None):
        ...
        self.model_id = model_id or os.getenv("HF_MODEL", "cardiffnlp/twitter-roberta-base-sentiment")
    ...
    def _to_item(self, probs: Dict[str, float], degraded: bool = False) -> Dict[str, Any]:
        ...
        if self.backend == "local":
            ...
            try:
                ...
                for i in range(len(texts)):
                    results[i] = self._to_item({"pos": 0.0, "neu": 1.0, "neg": 0.0}, degraded=True)
            ...
            with ThreadPoolExecutor(max_workers=self.concurrency) as executor:
                futures = [executor.submit(call_one, (i, text)) for i, text in enumerate(texts)]
