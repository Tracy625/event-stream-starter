...
from api.schemas.topic import TopicMergeConfig, TopicSignalResponse
...
def _redis_get_counts(topic_id: str, minutes: int) -> Optional[List[int]]:
    ...
    ts_list = [
        now - timedelta(minutes=i) for i in range(minutes, -1, -1)
    ]  # Include endpoints
...
class TopicAnalyzer:
    ...
    async def analyze_topic(
        self, topic_id: Optional[str] = None, entities: Optional[List[str]] = None
    ) -> TopicSignalResponse:
