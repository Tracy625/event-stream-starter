...
class DeduplicationService:
    ...
    def record(self, event_key: str, ts: Optional[datetime] = None) -> None:
        ...
        if self.redis_client:
            ...
            try:
                ...
                self.redis_client.setex(redis_key, self.window_sec, str(current_ts))
                logger.debug(
                    f"Recorded {event_key} in Redis with TTL {self.window_sec}s"
                )
            except Exception as e:
    ...
    def prune(self) -> int:
        ...
        expired_keys = [
            key
            for key, ts in self.memory_store.items()
