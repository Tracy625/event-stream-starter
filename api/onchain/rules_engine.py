    ...
    for verdict_type in ['upgrade_if', 'downgrade_if']:
        ...
        for condition in conditions:
            ...
            if '>=' in condition:
                ...
                raise ValueError(f"Invalid condition format: {condition}. Only >= and <= are supported")
...
def _evaluate_condition(
    ...
    logger.debug(f"Evaluated {field} {op} {label}({threshold}): {feature_value} -> {result}")
    return result, None
    ...
    try:
        ...
        if features.window_min not in rules.windows:
            ...
            return Verdict(
                ...
                confidence=0.0,
                note="window_unsupported"
            )
        
        # Validate feature ranges
        if not (0 <= features.active_addr_pctl <= 1):
            logger.warning(f"active_addr_pctl out of range: {features.active_addr_pctl}")
            return Verdict(
                decision="insufficient",
                confidence=0.0,
                note="feature_out_of_range"
            )
        ...
        if downgrade_results and all(downgrade_results):
            ...
            confidence = min(1.0, 0.6 + 0.4 * (sum(downgrade_results) / len(downgrade_results)))
            ...
            confidence = min(1.0, 0.6 + 0.4 * (sum(upgrade_results) / len(upgrade_results)))
