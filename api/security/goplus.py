...
def evaluate_goplus_raw(goplus_raw: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    goplus_version = rules.get("goplus_version") or os.getenv("GOPLUS_VERSION", "GoPlus@v1.2")
    ...
    if goplus_raw.get("error") or goplus_raw.get("degrade"):
        status = "timeout" if "timeout" in str(goplus_raw.get("error", "")).lower() else "error"
        degradation = rules.get("degradation", {})
        config = degradation.get("on_timeout" if status == "timeout" else "on_error", {})
