...
class GoPlusClient:
    ...
    def __init__(
        ...
        if not any([self.access_token, self.api_key, (self.client_id and self.client_secret)]):
    ...
    def _get_client(self) -> httpx.Client:
        ...
        if self._client is None:
            ...
            auth = None
            if not (self.access_token or self.api_key) and (self.client_id and self.client_secret):
    ...
    def _build_headers(self) -> Dict[str, str]:
        ...
        if self.access_token:
            headers["Authorization"] = f"Bearer {self.access_token}"  # httpx 会用这个发请求，但日志里别打印
        elif self.api_key:
    ...
    def _request(self, method: str, endpoint: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.retry + 1):
            ...
            try:
                ...
                if attempt < self.retry:
                    log_json(stage="goplus.retry", attempt=attempt + 1, reason="timeout")
                ...
                if attempt < self.retry:
                    log_json(stage="goplus.retry", attempt=attempt + 1, reason="network_error")
                    time.sleep(backoff_times[min(attempt, len(backoff_times)-1)])
                    continue
        
        # All retries exhausted
        log_json(stage="goplus.error", error=last_error, retries_exhausted=True)
        raise GoPlusClientError(f"Request failed after {self.retry} retries: {last_error}")
    ...
    def approval_security(self, chain_id: str, address: str, type: str = "erc20") -> Dict[str, Any]:
