...
class XClient(ABC):
    ...
    @abstractmethod
    def fetch_user_tweets(self, handle: str, since_id: Optional[str] = None, limit: int = 20) -> list[Dict[str, Any]]:
        """
...
class GraphQLXClient(XClient):
    ...
    def fetch_user_profile(self, handle: str) -> Optional[Dict[str, Any]]:
        ...
        log_json(stage="x.avatar.error", error="graphql_profile_not_implemented", handle=handle)
        return None
    ...
    def ping(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "timeout", "rtt_ms": int((time.perf_counter() - t0) * 1000)}
        except Exception as e:
...
class ApifyXClient(XClient):
    ...
    def __init__(self):
        ...
        self.poll_sleep_s = max(0.1, float(int(os.getenv("APIFY_POLL_INTERVAL_MS", "800")) / 1000.0))
        self.metrics_req = metrics_core.counter("x_backend_request_total", "X backend requests by backend/op/status")
    ...
    def _fetch_items(self, client: httpx.Client, dataset_id: str, limit: int) -> List[Dict[str, Any]]:
        # Cap fetched items to protect cost: at most limit*2
        max_items = max(1, min( max(1, (limit or 20) * int(os.getenv("APIFY_ITEMS_MAX_MULTIPLIER", "2"))), 200))
    ...
    def fetch_user_tweets(self, handle: str, since_id: Optional[str] = None, limit: int = 20) -> list[Dict[str, Any]]:
        ...
        try:
            if not self.token:
                log_json(stage="x.fetch.degrade", backend="apify", reason="Missing APIFY_TOKEN")
            ...
            with httpx.Client(timeout=self.timeout_s) as client:
                ...
                if isinstance(limit, int) and limit > 0:
                    tweets = tweets[:limit]
                log_json(stage="x.fetch.success", backend="apify", count=len(tweets), handle=handle)
            ...
            log_json(stage="x.fetch.error", backend="apify", error=f"HTTP {e.response.status_code}")
            self._observe((time.perf_counter()-start)*1000, "err", op)
            return []
        except Exception as e:
    ...
    def fetch_user_profile(self, handle: str) -> Optional[Dict[str, Any]]:
        ...
        return {"handle": handle, "avatar_url": "", "ts": datetime.now(timezone.utc).isoformat().replace("+00:00","Z")}
    ...
    def ping(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with httpx.Client(timeout=2.0) as client:
                ...
                return {"status": "ok" if r.status_code < 500 else "fail", "rtt_ms": rtt, "code": r.status_code}
        except httpx.TimeoutException:
            return {"status": "timeout", "rtt_ms": int((time.perf_counter() - t0) * 1000)}
        except Exception as e:
            return {"status": "net", "error": str(e)[:120]}
...
class MultiSourceXClient(XClient):
    ...
    def __init__(self, backends: List[str]):
        ...
        self.backends: List[Tuple[str, XClient]] = [(b, get_x_client(b)) for b in self.backends_order]
        ...
        self.failover_counter = metrics_core.counter("x_backend_failover_total", "Total count of X backend failovers")
        self.req_counter = metrics_core.counter("x_backend_request_total", "X backend requests by backend/op/status")
        self.lat_hist = metrics_core.histogram("x_backend_latency_ms", "X backend latency in ms", buckets=[50,100,200,500,1000,2000,5000])
        self.degrade_counter = metrics_core.counter("x_backend_degrade_total", "Degrades when all backends fail")
        self.cache_ttl = int(os.getenv("X_CACHE_TTL_S", "180"))
        self.race_mode = (os.getenv("X_RACE_MODE", "false").lower() in ("1","true","yes","on"))
    ...
    def _note_result(self, backend: str, op: str, ok: bool, elapsed_ms: float, error: Optional[str] = None, degrade: bool = False, reason: Optional[str] = None):
        self.lat_hist.observe(elapsed_ms, labels={"backend": backend, "op": op})
        self.req_counter.inc(labels={"backend": backend, "op": op, "status": _status_label(ok, degrade)})
    ...
    def _try(self, backend: str, client: XClient, op: str, fn):
        ...
        try:
            ...
            self._note_result(backend, op, False, (time.perf_counter()-t0)*1000, error=str(e), reason=reason)
            log_json(stage="x.fetch.error", backend=backend, error=str(e), reason=reason)
            return None
    ...
    def fetch_user_tweets(self, handle: str, since_id: Optional[str] = None, limit: int = 20) -> list[Dict[str, Any]]:
        ...
        if self.race_mode and len(order) >= 2:
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as ex:
                ...
                futs.append(ex.submit(lambda: (b1, self._try(b1, c1, "tweets", lambda: c1.fetch_user_tweets(handle, since_id, limit)))))
                time.sleep(self.race_guard_ms / 1000.0)
                futs.append(ex.submit(lambda: (b2, self._try(b2, c2, "tweets", lambda: c2.fetch_user_tweets(handle, since_id, limit)))))
        ...
        if cached:
            ...
            self._note_result("multi", "tweets", ok=False, elapsed_ms=0.0, degrade=True, error="degraded_cache")
            return cached
        return []
    ...
    def fetch_user_profile(self, handle: str) -> Optional[Dict[str, Any]]:
        ...
        merge_enabled = os.getenv("X_FIELD_MERGE", "true").lower() in ("1","true","yes","on")
        ...
        if primary:
            diag = {"source_map": {k:v for k,v in src_map.items() if k != "_primary"}, "stale": False}
        ...
        if cached:
            ...
            self._note_result("multi", "profile", ok=False, elapsed_ms=0.0, degrade=True, error="degraded_cache")
