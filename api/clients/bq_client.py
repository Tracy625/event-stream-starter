...
class BQClient:
    ...
    def __init__(self):
        ...
        self.dataset_ro = os.getenv("BQ_DATASET_RO", "bigquery-public-data.crypto_ethereum")
    ...
    def _get_client(self):
        ...
        if self._client is None:
            ...
            try:
                ...
                log_json(stage="bq.import_error", error="google-cloud-bigquery not installed")
                return None
    ...
    def query(self, sql: str, params: Optional[Dict] = None, timeout_s: Optional[int] = None) -> Iterator[Any]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_str = str(e)
                is_transient = any(x in error_str.lower() for x in ["timeout", "deadline", "unavailable", "rate"])
    ...
    def probe_connectivity(self) -> Dict:
        ...
        """
        if self.backend == "off":
            return {"probe": 1, "dry_run_pass": False, "bq_bytes_scanned": 0, "cost_guard_hit": False}
    ...
    def freshness(self, dataset: str, chain: str) -> Dict:
        ...
        try:
            ...
            row = results[0]
            data_as_of = row.data_as_of.isoformat() if hasattr(row.data_as_of, 'isoformat') else str(row.data_as_of)
