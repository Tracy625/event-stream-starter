    ...
    try:
        ...
        async def _refine_with_timeout():
            return await asyncio.wait_for(
                loop.run_in_executor(
                    None, refine_evidence, reasons, {"hint": "rules.reasons"}
                ),
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                result = executor.submit(asyncio.run, _refine_with_timeout()).result(
        ...
        log_json(stage="rules.refine_degrade", reason="timeout", timeout_ms=timeout_ms)
        return reasons, False

    except Exception as e:
        log_json(stage="rules.refine_degrade", reason="exception", error=str(e))
        return reasons, False
