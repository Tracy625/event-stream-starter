...
def upgrade():
    with op.batch_alter_table('events') as tbl:
        ...
        tbl.add_column(sa.Column('refined_impacted_assets', sa.ARRAY(sa.Text()), server_default='{}'))
        tbl.add_column(sa.Column('refined_reasons', sa.ARRAY(sa.Text()), server_default='{}'))
        ...
        tbl.add_column(sa.Column('refine_ts', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()')))
...
def downgrade():
    ...
    with op.batch_alter_table('events') as tbl:
        for col in [
            'refined_type','refined_summary','refined_impacted_assets','refined_reasons',
            'refined_confidence','refine_backend','refine_latency_ms','refine_ok','refine_error','refine_ts'
        ]:
            tbl.drop_column(col)