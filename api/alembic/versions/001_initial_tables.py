...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "is_candidate", sa.Boolean(), server_default=sa.text("FALSE"), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["event_key"], ["events.event_key"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_signals_event_key_ts",
        "signals",
        ["event_key", sa.text("ts DESC")],
        unique=False,
    )
