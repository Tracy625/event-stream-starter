...
def upgrade() -> None:
    ...
    state_exists = bind.execute(sa_text("""
        SELECT 1
    ...
    """)).scalar()
    check_exists = bind.execute(sa_text("""
        SELECT 1
    ...
    if not state_exists or not check_exists:
        with op.batch_alter_table('signals') as batch_op:
            if not state_exists:
                batch_op.add_column(
                    sa.Column('state', sa.Text(), server_default='candidate', nullable=False)
                )
            if not check_exists:
