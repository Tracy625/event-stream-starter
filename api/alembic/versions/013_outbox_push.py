...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('pending','retry','done','dlq')", name='ck_push_outbox_status')
    )
    
    # Create indexes
    op.create_index('ix_push_outbox_status_next_try_at', 'push_outbox', ['status', 'next_try_at'])
