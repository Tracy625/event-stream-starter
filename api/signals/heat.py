...
def compute_heat(
    ...
    *,
    ...
    result = {
        ...
        "slope": None,
        "trend": "flat",
        "window": {"ten": 600, "thirty": 1800},  # seconds
        "degrade": False,
        "from_cache": from_cache,
        "asof_ts": (
            asof_ts.isoformat() if hasattr(asof_ts, "isoformat") else str(asof_ts)
        ),
    }
...
def persist_heat(
    ...
    *,
    ...
    # Read configuration
    if upsert is None:
        upsert = os.getenv("HEAT_PERSIST_UPSERT", "true").lower() in (
            "true",
            "1",
            "yes",
            "on",
    ...
    if strict_match is None:
        strict_match = os.getenv("HEAT_PERSIST_STRICT_MATCH", "true").lower() in (
            "true",
            "1",
            "yes",
            "on",
    ...
    if not event_key and token and not strict_match:
        allow_symbol_fb = os.getenv("HEAT_ALLOW_SYMBOL_FALLBACK", "false").lower() in (
            "1",
            "true",
            "yes",
            "on",
    ...
    try:
        ...
        try:
            row_exists = (
                db.execute(check_query, {"event_key": event_key}).fetchone() is not None
            )
