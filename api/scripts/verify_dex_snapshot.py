...
class DexVerifier:
    ...
    def _add_test_result(
        self, name: str, passed: bool, details: Dict[str, Any] = None
    ) -> None:
        """Add test result to collection"""
        result = {"name": name, "pass": passed, "details": details or {}}
        ...
        try:
            ...
            if resp.status_code != 200:
                self._add_test_result(
                    "normal_response",
                    False,
                    {"status_code": resp.status_code, "expected": 200},
                )
                return False
            ...
            if missing_fields:
                self._add_test_result(
                    "normal_response", False, {"missing_fields": missing_fields}
                )
                return False

            # Verify we got actual data
            has_data = (
                data.get("price_usd") is not None
                or data.get("liquidity_usd") is not None
            )
            ...
            self._add_test_result("normal_response", False, {"error": str(e)})
            return False
        ...
        try:
            ...
            if resp1.status_code != 200:
                self._add_test_result(
                    "cache_hit",
                    False,
                    {"error": "First request failed", "status": resp1.status_code},
                )
                return False
            ...
            if resp2.status_code != 200:
                self._add_test_result(
                    "cache_hit",
                    False,
                    {"error": "Second request failed", "status": resp2.status_code},
                )
                return False
            ...
            self._add_test_result("cache_hit", False, {"error": str(e)})
            return False
        ...
        try:
            ...
            if resp.status_code != 200:
                self._add_test_result(
                    "fallback",
                    False,
                    {"error": "Request failed", "status": resp.status_code},
                )
                return False
            ...
            fallback_occurred = (
                data.get("reason")
                in ["timeout", "conn_refused", "http_4xx", "http_5xx"]
                and data.get("source") in ["geckoterminal", "dexscreener"]
                and data.get("degrade") == False
            )

            # Even if no fallback, test passes if we got data
            success = fallback_occurred or (
                data.get("source") and not data.get("degrade")
            )
            ...
            self._add_test_result("fallback", False, {"error": str(e)})
            return False
        ...
        try:
            ...
            if resp.status_code not in [200, 503]:
                self._add_test_result(
                    "degrade",
                    False,
                    {"error": "Unexpected status code", "status": resp.status_code},
                )
                return False
            ...
            is_degraded = (
                data.get("degrade") == True
                and data.get("stale") == True
                and data.get("reason")
                in ["both_failed_last_ok", "both_failed_no_cache", "provider_error"]
            )
            ...
            self._add_test_result("degrade", False, {"error": str(e)})
            return False
    ...
    def test_both_failed(self) -> bool:
        ...
        try:
            ...
            if resp.status_code == 503:
                ...
                both_failed = (
                    data.get("price_usd") is None
                    and data.get("degrade") == True
                    and data.get("reason") in ["both_failed_no_cache", "provider_error"]
                )
            ...
            self._add_test_result(
                "both_failed",
                False,
                {"error": "Unexpected response", "status": resp.status_code},
            )
            return False

        except Exception as e:
            self._add_test_result("both_failed", False, {"error": str(e)})
            return False
