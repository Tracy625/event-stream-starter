...
def verify_onchain_features():
    ...
    with engine.connect() as conn:
        ...
        assert abs(actual_growth - expected_growth) < Decimal(
            "1e-8"
        ), f"growth_ratio mismatch: expected {expected_growth}, got {actual_growth}"
        log_json(
            stage="verify.growth",
            prev=prev_active,
            curr=curr_active,
            expected=float(expected_growth),
            actual=float(actual_growth),
        )
    ...
    assert (
        stats3["written"] == 0
    ), f"Expected 0 written (idempotent), got {stats3['written']}"
    assert (
        stats3["updated"] == 3
    ), f"Expected 3 updated (idempotent), got {stats3['updated']}"
