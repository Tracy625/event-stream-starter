...
def test_normal_operation():
    ...
    print(
        f"✓ Cache: {result['cache']}, Stale: {result['stale']}, Degrade: {result['degrade']}"
    )
...
def test_both_fail_with_last_ok():
    ...
    assert (
        result2["reason"] == "both_failed_last_ok"
    ), f"Wrong reason: {result2['reason']}"
    ...
    if result2["notes"]:
        assert any(
            "last_ok_from:" in note for note in result2["notes"]
...
def test_both_fail_no_cache():
    ...
    assert (
        result["price_usd"] is None or result["price"] is None
    ...
    assert (
        result["reason"] == "both_failed_no_cache"
