...
def verify_demo_scenarios():
    ...
    assert (
        "DEX 数据不足" in result2["reasons"]
    ), "DEMO2 should include DEX missing reason"
    ...
    assert (
        "情绪分析不可用" in result3["reasons"]
    ), "DEMO3 should include HF missing reason"
...
def verify_hot_reload():
    ...
    result = evaluator.evaluate({"goplus_risk": "green"}, {})
    print(
        f"   Version: {result['rules_version']}, Hot-reloaded: {result['hot_reloaded']}"
    )
...
def verify_env_substitution():
    ...
    test_events = {"last_sentiment_score": 0.5}  # < 0.6 (env), > 0.3 (default)
    ...
    assert any(
        "流动性不足" in r for r in result["reasons"]
    ), "Should detect liquidity below env threshold"
    assert any(
        "交易量过低" in r for r in result["reasons"]
    ), "Should detect volume below env threshold"
...
def verify_error_handling():
    ...
    assert result["level"] in [
        ...
        "caution",
        "opportunity",
    ], "Should return valid level even with errors"
