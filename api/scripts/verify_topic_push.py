...
def verify_topic_push():
    ...
    try:
        ...
        results["pass"] = (
            passed_checks >= 2 and  # At least 2 checks pass
            any(c["test"] == "message_format" and c["pass"] for c in results["checks"])
        )
    ...
    if mock_mode:
        ...
        print(f"  Check: {os.getenv('TELEGRAM_MOCK_PATH', '/tmp/telegram_sandbox.jsonl')}")
