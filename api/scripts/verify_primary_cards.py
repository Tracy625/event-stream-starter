...
def test_card_generation() -> Dict[str, Any]:
    ...
    try:
        ...
        signals = {
            ...
            "goplus_raw": {"summary": "Token security check passed"},
        }
...
def test_deduplication() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"pass": False, "test": "deduplication", "error": str(e), "details": {}}
...
def test_recheck_queue() -> Dict[str, Any]:
    ...
    try:
        ...
        test_items = [("test:high", 1), ("test:medium", 5), ("test:low", 10)]
        ...
        success = (
            len(items) == 3
            and items[0][0].decode() == "test:high"
            and items[0][1] == 1.0
            and items[1][0].decode() == "test:medium"
            and items[1][1] == 5.0
            and items[2][0].decode() == "test:low"
            and items[2][1] == 10.0
        )
        ...
        return {"pass": False, "test": "recheck_queue", "error": str(e), "details": {}}
...
def test_red_card_rules() -> Dict[str, Any]:
    ...
    try:
        ...
        success = (
            card["risk_level"] == "red"
            and "rules_fired" in card
            and len(card["rules_fired"]) > 0
            and "HONEYPOT" in card["risk_note"]
        )
        ...
        return {"pass": False, "test": "red_card_rules", "error": str(e), "details": {}}
...
def main():
    ...
    for test in tests:
        log_json(stage="verify.card.test", test=test["test"], pass_status=test["pass"])
