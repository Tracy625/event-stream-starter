...
def test_endpoint(base_url: str = "http://localhost:8000"):
    ...
    response = requests.get(
        f"{base_url}/rules/eval", params={"event_key": "eth:DEMO1:2025-09-10T10:00:00Z"}
    )
    ...
    if response.status_code == 200:
        ...
        ), f"Expected opportunity, got {data['level']}"
    ...
    response = requests.get(
        f"{base_url}/rules/eval", params={"event_key": "eth:DEMO2:2025-09-10T11:00:00Z"}
    )
    ...
    if response.status_code == 200:
        ...
        assert any(
            "DEX" in r for r in data["reasons"]
        ), "Should include DEX missing reason"
    ...
    response = requests.get(
        f"{base_url}/rules/eval", params={"event_key": "eth:DEMO3:2025-09-10T12:00:00Z"}
    )
    ...
    response = requests.get(
        f"{base_url}/rules/eval", params={"event_key": "nonexistent:key:2025"}
    )
...
def test_concurrent_requests(base_url: str = "http://localhost:8000"):
    ...
    def make_request(event_key: str, index: int):
        try:
            start = time.time()
            response = requests.get(
                f"{base_url}/rules/eval", params={"event_key": event_key}
            )
            ...
            errors.append({"index": index, "event_key": event_key, "error": str(e)})
    ...
    if results:
        ...
        for result in results[:3]:  # Show first 3
            print(
                f"  Request {result['index']}: {result['event_key'][:20]}... -> {result['level']} ({result['latency_ms']:.1f}ms)"
            )
