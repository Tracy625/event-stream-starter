    ...
    for i, sample in enumerate(samples, 1):
        try:
            ...
            if result.risk_label == sample["expected"]:
                print(
                    f"✓ Sample {i}: {sample['address'][:10]}... = {result.risk_label} (expected: {sample['expected']}) - PASS"
                )
                log_json(
                    stage="verify.sample",
                    sample=i,
                ...
                print(
                    f"✗ Sample {i}: {sample['address'][:10]}... = {result.risk_label} (expected: {sample['expected']}) - FAIL"
                )
                log_json(
                    stage="verify.sample",
                    sample=i,
