...
def get_upstream_features(
    chain: str, address: str, window_minutes: int, as_of_ts: datetime
...
def enrich_onchain_features(
    chain: str,
    address: str,
    windows: Tuple[int, ...] = (30, 60, 180),
    calc_version: int = 1,
    limit_prev_lookback: int = 1,
    ...
    try:
        for window_minutes in windows:
            ...
            if features.get("addr_active"):
                growth_ratio = compute_growth_ratio(
                    session,
                    chain,
                    address,
                    window_minutes,
                    features["addr_active"],
                    as_of_ts,
                )
