...
def fetch_due(session: Session, *, limit: int = 50) -> List[PushOutbox]:
    ...
    query = session.query(PushOutbox).filter(
        ...
        skip_locked=True  # Skip locked rows to prevent blocking
    ).limit(limit)
...
def mark_done(session: Session, *, row_id: int) -> None:
    ...
    """
    result = session.query(PushOutbox).filter(
    ...
    if result == 0:
        ...
        log_json(
            stage="outbox.mark_done",
            warning="No row updated",
            row_id=row_id
        )
...
def mark_retry(
    ...
    *,
    ...
    current = session.query(PushOutbox).filter(
        PushOutbox.id == row_id
    ).first()
    ...
    if new_attempt >= MAX_RETRY_ATTEMPTS:
        ...
        move_to_dlq(
            ...
            snapshot=current.payload_json if isinstance(current.payload_json, dict) else {"text": str(current.payload_json)}
        )
        return False
...
def move_to_dlq(
    ...
    *,
    ...
    if snapshot is None:
        snapshot = outbox.payload_json if isinstance(outbox.payload_json, dict) else {"text": str(outbox.payload_json)}
...
def recover_from_dlq(session: Session, *, row_id: int) -> bool:
    ...
    result = session.query(PushOutbox).filter(
        and_(
            PushOutbox.id == row_id,
            PushOutbox.status == OutboxStatus.DLQ
        )
    ).update({
        'status': OutboxStatus.RETRY,
    ...
    if result > 0:
        from api.core.metrics_store import log_json
        log_json(
            stage="outbox.recovered_from_dlq",
            row_id=row_id
        )
