        ...
        try:
            with open(config_path, "r") as f:
                ...
                if config and "kol" in config:
                    handles = [
                        kol["handle"] for kol in config["kol"] if "handle" in kol
                    ]
                    if handles:
...
def is_duplicate(
    redis_client: Optional[redis.Redis], tweet_id: str, fingerprint: str
) -> bool:
...
def run_once() -> Dict[str, int]:
    ...
    """
    stats = {"fetched": 0, "normalized": 0, "dedup_hit": 0, "inserted": 0}
