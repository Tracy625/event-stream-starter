...
def push_topic_to_telegram(topic_id: str):
    ...
    try:
        ...
        if not candidate_data:
            log_json(stage="push.topic.missing", topic_id=topic_id)
        ...
        if result.get("success"):
            ...
            log_json(
                stage="push.topic.failed", topic_id=topic_id, error=result.get("error")
            )
            return {"success": False, "error": result.get("error")}

    except Exception as e:
        log_json(stage="push.topic.error", topic_id=topic_id, error=str(e))
...
def format_topic_message(c: Dict[str, Any]) -> str:
    ...
    latest_iso = (
        (c.get("latest_ts") or "").isoformat()
        if hasattr(c.get("latest_ts"), "isoformat")
        else str(c.get("latest_ts", ""))
    )
    # 最小文案 + 风险提示（避免引导交易与仿冒）
    return (
        f"🔥 热点话题：{ents_show}\n"
...
def push_to_telegram(text: str) -> Dict[str, Any]:
    ...
    notifier = TelegramNotifier()
    chat_id = os.getenv("TELEGRAM_TOPIC_CHAT_ID") or os.getenv(
    ...
    result = notifier.send_message(chat_id=chat_id, text=text, parse_mode="HTML")
...
def push_topic_digest():
    ...
    try:
        ...
        if not digest_data:
            log_json(stage="push.digest.empty", date=today)
        ...
        if result.get("success"):
            log_json(stage="push.digest.sent", date=today, topic_count=len(digest))
            return {"success": True, "date": today}
        else:
            log_json(stage="push.digest.failed", date=today, error=result.get("error"))
            return {"success": False, "error": result.get("error")}

    except Exception as e:
        log_json(stage="push.digest.error", error=str(e))
        return {"success": False, "error": str(e)}
